import { type HttpClient, HttpStatusCode } from '@/core/data/protocols/http'
import {
  BadRequestError,
  ForbiddenError,
  UnexpectedError,
} from '@/core/domain/errors'

import type { CreateGeneralCultivationDiseaseUseCase } from '../../../domain/use-cases/general-cultivation-diseases-use-cases'

export class RemoteCreateGeneralCultivationDiseaseUseCase
  implements CreateGeneralCultivationDiseaseUseCase
{
  constructor(
    private readonly url: string,
    private readonly httpClient: HttpClient
  ) {}

  execute: CreateGeneralCultivationDiseaseUseCase['execute'] = async ({
    generalCultivationDisease,
  }) => {
    const { statusCode } = await this.httpClient.request({
      url: this.url,
      method: 'post',
      body: generalCultivationDisease,
    })

    if (statusCode === HttpStatusCode.created) return

    if (statusCode === HttpStatusCode.badRequest) throw new BadRequestError()

    if (statusCode === HttpStatusCode.forbidden) {
      throw new ForbiddenError(
        'Você não tem permissão para criar uma doença de cultivo geral.'
      )
    }

    throw new UnexpectedError()
  }
}
