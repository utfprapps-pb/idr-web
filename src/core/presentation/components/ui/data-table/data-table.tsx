...
export function TableBody<TData extends RowData>({
  ...
  // ALTERAÇÃO 1: Inverte a ordem dos 'if's.
  // Prioriza a mensagem de "sem resultados" apenas depois que o loading estiver completo.
  if (!loading && !rowModel.rows.length) {
    return (
      <Table.Row>
  ...
  // ALTERAÇÃO 2: Mantém o 'if' de loading sozinho.
  // A lógica de carregamento deve ser a primeira a ser verificada.
  if (loading) {
    return (
      <Table.Row>
  ...
  function handleOnClickRow(event: MouseEvent, row: TData) {
